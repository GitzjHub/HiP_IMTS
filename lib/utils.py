...
def split_and_patch_batch(data_dict, args, n_observed_tp, patch_indicesList):
	...
	split_dict = {"tp_to_predict": data_dict["tp_to_predict"].clone(),
			...
			"data_max": data_dict["data_max"].clone(),
			"data_min": data_dict["data_min"].clone(),
			}
	...
	observed_tp_patches_List = []
	observed_data_patches_List = []
	observed_mask_patches_List = []

	for index in range(args.nscale):
		patch_indices = patch_indicesList[index]

		observed_tp_patches = observed_tp.view(1, 1, -1, 1).repeat(n_batch, args.npatches[index], 1, n_dim)
		observed_data_patches = observed_data.view(n_batch, 1, n_tp, n_dim).repeat(1, args.npatches[index], 1, 1)
		observed_mask_patches = observed_mask.view(n_batch, 1, n_tp, n_dim).repeat(1, args.npatches[index], 1, 1)

		max_patch_len = 0
		for i in range(args.npatches[index]):
		...
		patch_indices_fianl = torch.full((n_batch, args.npatches[index], max_patch_len, n_dim), n_tp).to(device)
		...
		pad_zeros_data = torch.zeros([n_batch, args.npatches[index], 1, n_dim]).to(device)
		observed_tp_patches = torch.cat([observed_tp_patches, pad_zeros_data], dim=2).gather(2, patch_indices_fianl)
		...
		observed_tp_patches_List.append(observed_tp_patches)
		observed_data_patches_List.append(observed_data_patches)
		observed_mask_patches_List.append(observed_mask_patches)
	
	split_dict["observed_tp_List"] = observed_tp_patches_List
	split_dict["observed_data_List"] = observed_data_patches_List
	split_dict["observed_mask_List"] = observed_mask_patches_List
