...
class PhysioNet(object):
	...
	urls = [
		'https://physionet.org/files/challenge-2012/1.0.0/set-a.tar.gz_download',
		'https://physionet.org/files/challenge-2012/1.0.0/set-b.tar.gz_download',
		'https://physionet.org/files/challenge-2012/1.0.0/set-c.tar.gz_download',
	]
	...
	def download(self):
		...
		for url in self.urls:
			filename = url.rpartition('/')[2] # filename = set-a.tar.gz_download
...
def get_data_min_max(records, device):
	...
	print('data_max:', data_max.mean())
	print('data_min:', data_min.mean())
	print('time_max:', time_max.mean())
...
def patch_variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	patch_indicesList = []
	for index, patch_size in enumerate(args.patch_sizes):
		patch_indices = []
		st, ed = 0, patch_size
		for i in range(args.npatches[index]):
			if(i == args.npatches[index]-1):
				inds = torch.where((observed_tp >= st) & (observed_tp <= ed))[0]
			else:
				inds = torch.where((observed_tp >= st) & (observed_tp < ed))[0]
			patch_indices.append(inds)
			st += args.strides[index]
			ed += args.strides[index]

		patch_indicesList.append(patch_indices)
	...
	data_dict = utils.split_and_patch_batch(data_dict, args, n_observed_tp, patch_indicesList)
